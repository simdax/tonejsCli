...
<template>
  <main>
    ...
    <bob>
      <p>rythme {{rythmes}} </p>
      <input type="number" v-for="(rythme,i) in rythmes" v-model.number="rythmes[i]" @input="update"/>
      <p>note {{notes}} </p>
      <input type="number" v-for="(note,i) in notes" v-model.number="notes[i]" @input="update"/>
    </bob>
...
<script>
	   ...
	   methods: {
	       update () {},
	       set (val, obj, ind) {
		   (obj == 'notes') ? this.mel.notes[ind] = parseInt(val) : this.mel.rythmes[ind] = parseInt(val)
		   console.log(this.mel[obj][ind], val, obj, ind)
		   console.log(this.mel.rythmes, this.mel.notes)
	       },
	       keys () {
